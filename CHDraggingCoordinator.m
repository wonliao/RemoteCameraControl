...
- (CGPoint)_destinationPointForReleasePoint:(CGPoint)releasePoint
{
    ...
    if (self.snappingEdge == CHSnappingEdgeBoth){   //ChatHead will snap to both edges
        if (destinationEdge == CGRectMinXEdge) {
            destinationX = CGRectGetMinX(dropArea) + midXDragView - 10;
        } else {
            destinationX = CGRectGetMaxX(dropArea) - midXDragView - 10;
        }
...
- (void)draggableViewTouched:(CHDraggableView *)view
{
    ...
    if (_state == CHInteractionStateNormal) {
        ...
        [appDelegate bluetoothStartSend];
        ...
        [appDelegate bluetoothStopSend];
    }
}


- (void)closeDraggableView:(CHDraggableView *)view
{
    AppDelegate* appDelegate = (AppDelegate*)[[UIApplication sharedApplication] delegate];
...
- (void)_presentViewControllerForDraggableView:(CHDraggableView *)draggableView
{
    ...
    _presentedNavigationController.view.frame = CGRectMake([self _navigationControllerFrame].origin.x,
                                                           0,
    ...
    [_presentedNavigationController setNavigationBarHidden:YES animated:NO];
    
    
    //[self.window insertSubview:_presentedNavigationController.view belowSubview:draggableView];
    [self.window.rootViewController.view insertSubview:_presentedNavigationController.view belowSubview:draggableView];
